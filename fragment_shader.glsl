...
{
    float k = sin(time)*bend; // or some other amount
    float c = cos(k*p.x);
    float s = sin(k*p.x);
    mat2  m = mat2(c,-s,s,c);
...
vec2 sdfWorld(in vec3 pos){
    ...
    float bheight = 2.0+10.0*abs(sin(time+rnd(bid)*.25));
    ...
    pos -= vec3(.0,map(time,.0,T_INTRO*.8,15.,1.),-15.0);
...
vec3 getColor(vec3 pos, vec3 nor,vec3 rd, float material_id){
    ...
    vec3 sun_pos = normalize(vec3(
        map(time,T_SUNRISE*.25,T_SUNRISE,-6.0,1.0),
        map(time,T_SUNRISE*.25,T_SUNRISE,-3.0,3.0),
        3.0));
    ...
    float sunrise = map(time,T_SUNRISE*.25,T_SUNRISE,.01,1.0);
...
vec3 render(in vec2 p){
    ...
    if(SCENE==SCENE_INTRO){
        ...
        cam_y = map(time,0.0,T_INTRO*.8,25.0,2.0);
        aim_y = map(time,0.0,T_INTRO*.8,-2.0,0.0);
    }else
    if(SCENE==SCENE_HELI){
        cam_fov = CAM_LENS_24mm;
        cam_x = 2.;
        cam_y = 15.+sin(time*.2)*.1;
        aim_y = -2.;
        aim_z = -.5-abs(sin(time*.2))*.5;
    }else
    ...
    // sky simulation
    vec3 col = vec3(map(time,T_SUNRISE*.5,T_SUNRISE,1.0,.0),0.75,1.0) - 0.5*rd.y;
    col =  mix(col, vec3(0.7,0.8,0.8), exp(-10.0*rd.y));
    col *= map(time,T_SUNRISE*.25,T_SUNRISE,.0,1.0);
    ...
    if (ray_hit>0.0){
        ...
        vec3  fogColor  = FOG_COLOR* map(time,T_SUNRISE*.25,T_SUNRISE,.01,1.0);
